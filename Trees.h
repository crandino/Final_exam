...
public:
	...
	void witdhIterative(DList<TreeNode<TREEDATA>*> *list)
	{
		Stack<TreeNode<TREEDATA>*> main_stack;
		Stack<TreeNode<TREEDATA>*> sons_stack;
		TreeNode<TREEDATA> *node = &root;
		main_stack.push(node);

		while (main_stack.getElements() != 0)
		{
			main_stack.pop(node);
			list->add(node);

			doubleNode<TreeNode<TREEDATA>*> *item = node->children.getFirst();
			for (; item != NULL; item = item->next)
			{
				sons_stack.push(item->data);
			}

			if (main_stack.getElements() == 0)
			{
				while (sons_stack.getElements() != 0)
				{
					sons_stack.pop(node);
					main_stack.push(node);
				}
